...
end program EigenOpenACCTest

subroutine EigenOpenACC
use testUtils
implicit none
...
! Get buffer size
!$acc enter data copyin( a ), create( w, work, iwork )
!$acc host_data use_device( a, w, work, iwork )
   call dsyevd( jobz, uplo, n, a, n, w, work, -1, iwork, -1, info )
!$acc end host_data
!$acc exit data copyout( work, iwork )
...
! Call solver
call startTimer()
!$acc enter data create( work, iwork )
!$acc host_data use_device( a, w, work, iwork )
   call dsyevd( jobz, uplo, n, a, n, w, work, lwork, iwork, liwork, info )
!$acc end host_data
!$acc exit data copyout( a, w ) delete( work, iwork )
call endTimer()
