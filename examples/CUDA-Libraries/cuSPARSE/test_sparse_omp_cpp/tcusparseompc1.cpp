 ...
 * Modified for OpenMP from the CUDA Samples example ConjugateGradientUM
 *
 */
...
int main(int argc, char **argv)
{
    ...
    #pragma omp target enter data map( to:rhs[0:N], I[0:N+1], J[0:nz], val[0:nz], x[0:N], r[0:N] ) map( alloc: p[0:N], Ax[0:N] )
    ...
    #pragma omp target data use_device_ptr( val, I, J, x, Ax, p, r )
    {
        ...
        buffer = omp_target_alloc(bsize, 0);
    ...
    #pragma omp target exit data map( from:x[0:N] ) map( delete:rhs[0:N], I[0:N+1], J[0:nz], val[0:nz], r[0:N], p[0:N], Ax[0:N] )
