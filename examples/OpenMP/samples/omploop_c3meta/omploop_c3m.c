...
#pragma omp metadirective when(user={condition(use_gpu)}: target teams loop collapse(2) map(tofrom:b[:n*m],a[:n*m])) \
...
main( int argc, char* argv[] )
{
	...
	if( argc > 2 ){
		...
		if( argc > 4 ){
			...
			!strcmp( argv[4], "HOST" ) ){
			use_gpu=false;
			printf( "targeting multicore CPU\n" );
		    }else
		    if( !strcmp( argv[4], "gpu" ) ||
			!strcmp( argv[4], "GPU" ) ){
			printf( "targeting GPU\n" );
		    }else{
    ...
    w1 = 0.3;
    w2 = 0.2;
    #pragma omp metadirective when(user={condition(use_gpu)}: target data map(tofrom:bb[:n*m],aa[:n*m])) 
    {
    gettime( &t1 );
    ...
    printf( "%13ld microseconds parallel\n", cgpu );
    printf( "%13ld microseconds serial\n", chost );
